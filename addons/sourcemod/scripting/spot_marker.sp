...
public APLRes AskPluginLoad2(Handle myself, bool bLateLoad, char[] szError, int szErrorLen)
{
    ...
    g_bIsL4D2 = engine == Engine_Left4Dead2;
...
public void OnPluginStart()
{
    ...
    g_cvVictimDeny         = CreateConVar("sm_spot_marker_victim_deny",          "1",                      "Allow the command to be used only without grabbers.\n0 = OFF, 1 = ON", _, true, 0.0, true, 1.0);
    ...
    HookConVarChange(g_cvAliveOnly,          OnCvChanged_VictimDeny);
    ...
    g_bVictimDeny = GetConVarBool(g_cvVictimDeny);
...
void OnCvChanged_VictimDeny(ConVar cv, const char[] szOldValue, const char[] szNewValue) {
    g_bVictimDeny = GetConVarBool(cv);
}
...
{
    g_fSpriteSpeed = GetConVarFloat(cv);
    g_bSpriteSpeed = (g_fSpriteSpeed > 0.0);
}
...
Action Cmd_Mark(int iClient, int iArgs)
{
    ...
    if (g_bVictimDeny && IsClientVictim(iClient)) {
        return Plugin_Handled;
    }
...
bool IsClientVictim(int iClient)
{
    /* Hunter */
